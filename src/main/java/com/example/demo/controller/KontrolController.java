...
public class KontrolController {
    ...
    public ResponseEntity<?> deneme() {
        ...
        List<KontrolController.InnerEtkinlik> innerEtkinlikList = new ArrayList<>();
        for(Etkinlik e:etkinlikarray){
            innerEtkinlikList.add(new KontrolController.InnerEtkinlik(e.getId(),e.getAciklama(),e.getIsim(),
                    e.getKatılım_sayisi(),e.getTarih().toString(),e.getDikim_alani().getIsim()));
        }
        return new ResponseEntity<List<KontrolController.InnerEtkinlik>>(innerEtkinlikList, HttpStatus.OK);
